...
> Chaining model calls in sequence degrades reliability: each step conditions on the previous step’s output, so errors are inherited rather than cancelled. As a first-order idealization, if each of N steps succeeds independently with probability p, end-to-end success is p^N — a ten-step chain at 95% per-step accuracy yields roughly 60% end-to-end (0.95¹⁰ ≈ 0.599). This independent-error model is a lower-bound intuition, not a measurement. Sinha et al. (arXiv:2509.09677, ICLR 2026) document *self-conditioning*, in which per-step accuracy falls as a trajectory lengthens, so real errors are positively correlated and decay can exceed what p^N predicts; conversely, retries and verification gates raise the effective per-step reliability. METR (arXiv:2503.14499) gives the empirical task-horizon picture, and Cemri et al. (MAST, arXiv:2503.13657) find that multi-step agent failures are driven more by system design and inter-agent misalignment than by base-model quality. Kim et al. (*Towards a Science of Scaling Agent Systems*, arXiv:2512.08296) measure this directly: across 180 configurations on four agentic benchmarks, independent agents with no inter-agent verification amplified errors 17.2× relative to a single-agent baseline, while centralized coordination — an orchestrator that reviews sub-agent output before aggregation — contained amplification to 4.4×. The 4× difference is the presence or absence of a verification bottleneck, not a difference in the underlying models. The architectural response is to insert deterministic steps — contracts, finite-state checks, hard-coded policies — that halt the compounding loss: a deterministic gate at a critical step resets reliability for that checkpoint.
...
This is a falsifiable claim, and it is designed to be tested directly. Experiment 03 in `docs/thesis-experiments.md` holds the gates fixed and swaps the model across tiers (Axis A), then holds the model fixed and varies gate strictness (Axis B); the thesis predicts roughly flat metrics on Axis A and sharply varying metrics on Axis B, and is falsified if Axis A metrics correlate strongly with model size. That experiment's harness is **built and mock-validated but not yet run with real models** — the Stage 02 synthesis agent exists at `experiments/03-model-swap/synthesize.py`, while the `proof/` pipeline implements only Stage 00 ingestion. Beyond not being run yet, the claim faces a second limit: Experiment 03 exercises the harness's own gate ladder in `experiments/03-model-swap/`, not the production contract spine at `_config/stage-contract.py`. A flat Axis A would therefore support the architectural claim — that *a* deterministic gate layer, not the model, governs reliability — while leaving direct validation of the production `stage-contract.py` gate as open work.
...
- Kim, Y., Gu, K., Park, C., Schmidgall, S., et al. (2025). *Towards a Science of Scaling Agent Systems.* arXiv:2512.08296.
